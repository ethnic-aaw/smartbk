...
$id = (int) ($_GET['id'] ?? 0);
$jenis = db_fetch('SELECT * FROM jenis_pelanggaran WHERE id = ? LIMIT 1', [$id], 'row');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($old['kode'] === '') {
        ...
        $dup = db_fetch('SELECT id FROM jenis_pelanggaran WHERE kode = ? AND id <> ? LIMIT 1', [$old['kode'], $id], 'row');
        if ($dup) {
            $errors['kode'] = 'Kode sudah digunakan jenis lain.';
        }
    }
    ...
    if (!$errors) {
        $ok = db_query(
            ...
            [$old['kode'], $old['nama'], $old['komponen'], $old['kategori'], $poin, $old['deskripsi'] ?: null, $old['konsekuensi'] ?: null, $id]
        );

        if ($ok) {
            set_flash('success', 'Jenis pelanggaran "' . $old['nama'] . '" berhasil diperbarui.');
            redirect_to(rtrim(APP_BASE, '/') . '/pelanggaran/master.php');
        }
    ...
    <form method="post">
        ...
        <div class="form-grid">
            <div class="form-group">
                <label>Kode</label>
                <input type="text" name="kode" value="<?= e($old['kode']) ?>" class="<?= isset($errors['kode']) ? 'input-invalid' : '' ?>">
